...
void dummy(double *);
extern void kernel_loop(double *a, double *b, double c0, double c1, double c2, double c3, int M, int P, int N);
int main(int argc, char **argv)
{
  ...
  double c0;
  c0 = (rand() / ((double ) RAND_MAX)) / 6.0;
  double c1;
  c1 = (rand() / ((double ) RAND_MAX)) / 6.0;
  double c2;
  c2 = (rand() / ((double ) RAND_MAX)) / 6.0;
  double c3;
  c3 = (rand() / ((double ) RAND_MAX)) / 6.0;
  int repeat = 1;
  double runtime = 0.0;
  double wct_start;
  double wct_end;
  ...
  while ((runtime < 2.0) || (repeat <= 2))
  {
    ...
    {
      kernel_loop(a, b, c0, c1, c2, c3, M, N, P);
      tmp = a;
      a = b;
      b = tmp;
    }
